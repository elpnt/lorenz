...
const buttonStyles = tv({
	...
	variants: {
		...
		intent: {
			primary: [
				...
				"data-active:after:bg-(--btn-hover-overlay) hover:after:bg-(--btn-hover-overlay)",
			...
			outline: [
				...
				// Icon
				"[--btn-icon:var(--color-zinc-500)] data-active:[--btn-icon:var(--color-zinc-700)] hover:[--btn-icon:var(--color-zinc-700)] dark:data-active:[--btn-icon:var(--color-zinc-400)] dark:hover:[--btn-icon:var(--color-zinc-400)]",
			],
			plain: [
				// Base
	...
	// compoundVariants: [{isDanger: true, size: ["md"], }],
	defaultVariants: {
		intent: "primary",
		size: "md",
		isDanger: false,
	...
	return (
		<AriaButton
			...
			className={composeRenderProps(className, (className, renderProps) =>
				buttonStyles({ ...renderProps, intent, size, isDanger, className }),
			)}
		>
			{(values) => (
